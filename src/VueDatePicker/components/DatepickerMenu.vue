<template>
    <transition appear :name="defaults.transitions?.menuAppear" :css="!!transitions">
        <div :id="uid ? `dp-menu-${uid}` : undefined" tabindex="0" ref="dpMenuRef" role="dialog" :class="dpMenuClass"
            @mouseleave="clearHoverDate" @click="handleDpMenuClick" @keydown.esc="handleEsc"
            @keydown.left.prevent="handleArrowKey('left')" @keydown.up.prevent="handleArrowKey('up')"
            @keydown.down.prevent="handleArrowKey('down')" @keydown.right.prevent="handleArrowKey('right')"
                ...
                <div class="dp__preset_ranges" v-if="presetRanges?.length">
                    <div v-for="(preset, i) in presetRanges" :key="i" :style="preset.style || {}" class="dp__preset_range"
                        ...
                        <template v-if="preset.slot">
                            <slot :name="preset.slot" :preset-date-range="presetDateRange" :label="preset.label"
                                :range="preset.range" />
                        </template>
                        <template v-else>
                            {{ preset.label }}
                ...
                <div class="dp__instance_calendar" ref="calendarWrapperRef" role="document">
                    <div :class="menuCalendarClassWrapper">
                        <div v-for="(instance, i) in calendarAmm" :key="instance" :class="calendarInstanceClassWrapper">
                            <MonthYearPicker :ref="(el: any) => {
                                    if (el) monthYearPickerRefs[i] = el;
                                }
                                " v-if="!disableMonthYearSelect && !timePicker" :months="months" :years="years"
                                :month="month(instance)" :year="year(instance)" :instance="instance"
                                :internal-model-value="internalModelValue" v-bind="baseProps"
                                @mount="childMount('monthYearInput')" @reset-flow="resetFlow"
                                @update-month-year="updateMonthYear(instance, $event)" @month-year-select="monthYearSelect"
                            ...
                            <Calendar :ref="(el: any) => {
                                ...
                                " :specific-mode="specificMode" :get-week-num="getWeekNum" :instance="instance"
                                :mapped-dates="mappedDates(instance)" :month="month(instance)" :year="year(instance)"
                                v-bind="baseProps" @select-date="selectDate($event, !isFirstInstance(instance))"
                                @handle-space="handleSpace($event, !isFirstInstance(instance))"
                                @set-hover-date="setHoverDate($event)" @handle-scroll="handleScroll($event, instance)"
                                @handle-swipe="handleSwipe($event, instance)" @mount="childMount('calendar')"
                                @reset-flow="resetFlow" @tooltip-open="$emit('tooltip-open', $event)"
                    ...
                    <div>
                        ...
                        <template v-else>
                            <TimePickerCmp v-if="enableTimePicker && !monthPicker && !weekPicker" ref="timePickerRef"
                                :hours="time.hours" :minutes="time.minutes" :seconds="time.seconds"
                                :internal-model-value="internalModelValue" v-bind="baseProps"
                                @mount="childMount('timePicker')" @update:hours="updateTime($event)"
                                @update:minutes="updateTime($event, false)"
                                @update:seconds="updateTime($event, false, true)" @reset-flow="resetFlow"
                                @overlay-closed="onTimePickerClose" @overlay-opened="$emit('time-picker-open', $event)"
            ...
            <ActionRow v-if="!autoApply || keepActionRow" :menu-mount="menuMount" :calendar-width="calendarWidth"
                :internal-model-value="internalModelValue" v-bind="baseProps" @close-picker="$emit('close-picker')"
                @select-date="$emit('select-date')" @invalid-select="$emit('invalid-select')"
